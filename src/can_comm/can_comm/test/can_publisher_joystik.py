...
class CanPublisher(Node):
    def __init__(self):
        ...
        self.device_path = "/dev/input/event24"  # NEOCON 장치 경로
        ...
        if r:
            for event in self.device.read():
                if event.type == evdev.ecodes.EV_KEY:
                    if event.code == evdev.ecodes.BTN_WEST:  # BTN_WEST: 전진/후진 모드
                        if event.value == 1:
                            self.set_go_mode()
                    elif event.code == evdev.ecodes.BTN_EAST:  # BTN_EAST: 회전 모드
                        if event.value == 1:
                            self.set_turn_mode()
                    elif event.code == evdev.ecodes.BTN_NORTH:  # BTN_NORTH: 시작
                        if event.value == 1:
                ...
                if event.type == evdev.ecodes.EV_ABS:
                    if event.code == evdev.ecodes.ABS_Y:  # 상하 조작
                        abs_y_value = event.value
                        if self.mode == "GO":  # 전진/후진 모드일 때
                            self.update_speed_based_on_joystick_updown(abs_y_value)
                    elif event.code == evdev.ecodes.ABS_Z:  # 좌우 조작
                        abs_z_value = event.value
                        if self.mode == "TURN":  # 회전 모드일 때
                            self.update_speed_based_on_joystick_rightleft(abs_z_value)

    def set_go_mode(self):
        self.mode = "GO"
        self.get_logger().info("Mode set to: GO (Forward/Backward)")

    def set_turn_mode(self):
        self.mode = "TURN"
        self.get_logger().info("Mode set to: TURN (Left/Right)")
